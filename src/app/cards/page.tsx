...
export default function Page() {
  ...
  const [state, dispatch] = useReducer<React.Reducer<ChangePinState, Partial<ChangePinState>>>(
    (state: ChangePinState, payload: Partial<ChangePinState>) => ({ ...state, ...payload }),
  ...
  const handleChangePinSubmit = async ({ pin, cardId }: { pin?: string; cardId?: string }) => {
    ...
    await changePin({ pin: pin ?? state.newPin, cardId: cardId ?? state.cardId! })
  ...
  useCopilotAction({
    name: "addNewCard",
    description: "Add new credit card, only admins can do it",
    parameters: [
      {
        name: "type",
        type: "string",
  ...
  useCopilotAction({
    ...
    render: ({ status, args }) => {
      ...
      if (status === 'inProgress') {
        return 'Fetching...'; // Your own component for loading state
      } else if (!filteredTransactions) {
        return 'Problem fetching transactions'
      } else {
        return (
            <TransactionsList transactions={filteredTransactions} compact />
        );
      }
    },
  });
  ...
  useCopilotAction({
    ...
    description: `
      This operation is per department and for admins only.
    ...
    renderAndWait: ({ args, handler, status }) => {
      ...
      if (!transactionId) {
        handler?.('A transaction ID was not given, could be that there arent any pending approval or there was an error')
      ...
      return (
          <TransactionsList
              ...
              approvalInterfaceProps={{
                onApprove: transactionId => handleChangeTransactionStatus({ id: transactionId, status: 'approved' }),
                onDeny: transactionId => handleChangeTransactionStatus({ id: transactionId, status: 'denied' }),
              }}
          />
      )
    }
  ...
  useCopilotChatSuggestions({
    instructions: `
      suggest random actions/information in this page related to credit cards, transactions or policies.
